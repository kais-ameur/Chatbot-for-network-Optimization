...
class ActionDecisionTree(Action):
    ...
    def run(
        ...
        client = MongoClient()
        db = client["decision_trees_type"]
        # Retrieve the decision tree type from the slot
        decision_tree_type = tracker.get_slot("decision_tree_type")
        if not decision_tree_type:
            dispatcher.utter_message("Please choose the area of your problem")
            return []
        # Retrieve the decision tree
        if decision_tree_type == "retainability":
            collection = db["retainability"]
            decision_tree = collection.find_one({"_id": ObjectId("64495bf84091063a58a1ecbd")})["Retainability"]
        elif decision_tree_type == "accessibility":
            collection = db["accessability"]
            decision_tree = collection.find_one({"_id": ObjectId("6448ee2f4091063a58a1ecb7")})["Accessability"]
        elif decision_tree_type == "mobility":
            collection = db["mobility"]
            decision_tree = collection.find_one({"_id": ObjectId("64494c194091063a58a1ecb9")})["Mobility"]
        elif decision_tree_type == "integrity":
            collection = db["integrity"]
            decision_tree = collection.find_one({"_id": ObjectId("644954374091063a58a1ecbb")})["Integrity"]
        else:
            dispatcher.utter_message("Invalid decision tree type.")
            return []

        # Get the selected option from the tracker slot
        selected_option = tracker.get_slot("selected_option")

        if not selected_option:
            # Show the root options as buttons
            buttons = [{"title": option["option"], "payload": f"/choose_option{{\"option\": \"{option['option']}\"}}" } for option in decision_tree["options"]]
            dispatcher.utter_button_message("Please select an option:", buttons)
        else:
            # Find the selected option in the decision tree
            node = decision_tree
            for option in selected_option.split(','):
                if "options" in node:
                    subnode = next((n for n in node["options"] if n["option"] == option), None)
                    if subnode:
                        node = subnode
                    else:
                        dispatcher.utter_message("Invalid option, please try again.")
                        return [SlotSet("selected_option", None)]
                else:
                    # This is a leaf node, return the action plan
                    action_plan = node["Action plan"]
                    dispatcher.utter_message(f"Action plan: {action_plan}")
                    tracker.slots["selected_option"] = None
                    if "Action plan" in node:
                        # Clear the decision tree type slot if the current node is a leaf node
                        return [SlotSet("selected_option", None)]
            if "options" in node:
                # Show the available options as buttons
                buttons = [{"title": option["option"], "payload": f"/choose_option{{\"option\": \"{selected_option},{option['option']}\"}}" } for option in node["options"]]
                dispatcher.utter_button_message("Please select an option:", buttons)
            else:
                # This is a leaf node, return the action plan
                action_plan = node["Action plan"]
